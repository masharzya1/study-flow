...
const Subjects = () => {
  ...
  const handleTopicToggle = useCallback((subjectId: string, chapterId: string, topicId: string, topicNameStr: string, difficulty: number) => {
    ...
    if (wasCompleted) {
      ...
      setVictoryData({ show: true, topicName: topicNameStr, xpGained, newLevel, isLevelUp });
    }
    ...
    <div className="p-4 md:p-8 max-w-3xl mx-auto space-y-5 pb-28 md:pb-8">
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-0.5">{t("subjects.subtitle")}</p>
        </div>
        <button
          onClick={() => setShowCreate(true)}
          className="flex items-center gap-2 px-3 py-2 rounded-xl bg-foreground text-primary-foreground font-medium hover-lift text-sm"
        >
        ...
        {showCreate && (
          ...
          >
            <div className="flex items-center justify-between">
              <h3 className="font-semibold text-sm">{t("subjects.newSubject")}</h3>
            ...
            />
            <div>
              <p className="text-[11px] text-muted-foreground mb-2 uppercase tracking-wider">{t("subjects.icon")}</p>
            ...
            <div>
              <p className="text-[11px] text-muted-foreground mb-2 uppercase tracking-wider">{t("subjects.color")}</p>
              <div className="flex gap-2 flex-wrap">
      ...
      <div className="space-y-3">
        {state.subjects.length === 0 && !showCreate && (
          <div className="text-center py-20 text-muted-foreground">
            <p className="text-base">{t("subjects.noSubjects")}</p>
            <p className="text-sm mt-1">{t("subjects.createFirst")}</p>
          </div>
        )}
                      ...
                      {subject.chapters.map(chapter => {
                        ...
                        return (
                          <div key={chapter.id} className="rounded-xl bg-secondary/40">
                            ...
                            >
                              <ChevronRight className={`w-3 h-3 text-muted-foreground transition-transform flex-shrink-0 ${chapterExpanded ? "rotate-90" : ""}`} />
                              <span className="font-medium flex-1 min-w-0 truncate">{chapter.name}</span>
                              <span className="text-[11px] text-muted-foreground flex-shrink-0">{chapterComplete}/{chapterTopics.length}</span>
                            </button>
                            <AnimatePresence>
                              {chapterExpanded && (
                                  ...
                                  <div className="px-2 pb-3 space-y-0.5">
                                    {chapterTopics.map(topic => (
                                      ...
                                      >
                                        ...
                                        <span className={`flex-1 min-w-0 truncate ${topic.completed ? "line-through text-muted-foreground" : ""}`}>{topic.name}</span>
                                        <span className="text-[11px] text-muted-foreground flex-shrink-0">{topic.estimatedMinutes}m</span>
                                    ...
                                    {addingTopic === chapter.id ? (
                                      <div className="space-y-2 mt-1">
                                        <input
                                          ...
                                          className="w-full px-2 py-1.5 text-sm rounded-lg bg-secondary text-foreground placeholder:text-muted-foreground border-0 outline-none"
                                          autoFocus
                                        ...
                                        <div className="flex items-center gap-2">
                                          <select
                                            ...
                                            className="flex-1 px-2 py-1.5 text-xs rounded-lg bg-secondary text-foreground border-0"
                                          >
                                            {levels.map(d => (
                                              <option key={d.id} value={d.id}>{d.label}</option>
                                            ))}
                                          </select>
                                          <button onClick={() => createTopic(subject.id, chapter.id)} className="text-foreground text-xs font-medium px-2 py-1.5 bg-foreground/10 rounded-lg">{t("subjects.add")}</button>
                                          <button onClick={() => setAddingTopic(null)} className="text-muted-foreground text-xs px-2 py-1.5">{t("subjects.cancel")}</button>
                                        </div>
                                      </div>
                                    ) : (
                                      <button
                                      ...
                                      >
                                        <Plus className="w-3 h-3" /> {t("subjects.addTopic")}
                                      </button>
                                    )}
                                  </div>
                                </motion.div>
                      ...
                      {addingChapter === subject.id ? (
                        <div className="flex gap-2">
                          <input
                            ...
                            className="flex-1 min-w-0 px-3 py-2 text-sm rounded-xl bg-secondary text-foreground placeholder:text-muted-foreground border-0 outline-none"
                            autoFocus
                            onKeyDown={e => e.key === "Enter" && createChapter(subject.id)}
                          />
                          <button onClick={() => createChapter(subject.id)} className="text-foreground text-sm font-medium flex-shrink-0">{t("subjects.add")}</button>
                          <button onClick={() => setAddingChapter(null)} className="text-muted-foreground text-sm flex-shrink-0">{t("subjects.cancel")}</button>
                        </div>
                      ) : (
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            className="flex items-center gap-1 text-[11px] text-muted-foreground hover:text-foreground transition-colors p-1.5"
                          >
                            <Plus className="w-3 h-3" /> {t("subjects.addChapter")}
                          </button>
                          <button
                          ...
                          >
                            <Trash2 className="w-3 h-3" /> {t("subjects.delete")}
                          </button>
                        </div>
                      )}
                    </div>
